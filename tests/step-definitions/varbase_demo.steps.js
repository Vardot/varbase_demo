...
/**
 * @file
 * Custom step definitions for the Varbase Demo test suite.
 *
 ...
 * (varbase-e2e does not ship a Drupal form-login step) and creating a Demo page
...
/**
 * Create and save a Demo page: fill the Title and the required Body field, then
 ...
 */
When(/^(?:I |we )?create a demo page titled "([^"]*)"$/, async function (title) {
  try {
    await gotoUrl(this.page, `${this.parameters.launchUrl}/node/add/varbase_demo_page`);
    ...
    throw friendly(`Could not create the Demo page "${title}"`, err);
  }
});
